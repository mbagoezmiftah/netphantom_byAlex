...
public static class NetworkHelper
{
    ...
    public static List<NetworkInterfaceInfo> GetInterfaces()
    {
        ...
        {
            // ILiveDevice tidak punya properti .Interface (itu hanya ada di kelas
            // konkret PcapDevice/LibPcapLiveDevice). MacAddress sudah tersedia langsung
            // lewat IPcapDevice, jadi tidak perlu cast atau akses .Interface sama sekali.
            var devMac = dev.MacAddress;
            if (devMac != null)
            {
                var mac = string.Join(":", devMac.GetAddressBytes().Select(b => b.ToString("X2")));
